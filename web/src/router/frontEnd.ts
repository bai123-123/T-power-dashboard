...
export async function initFrontEndControlRoutes() {
	...
	if (useUserInfo().userInfos.role_info.length <= 0) return Promise.resolve(true);
	// 添加动态路由
	await setAddRoute();
	...
	chil.forEach((route: any) => {
		if (route.meta.roles) {
			route.meta.roles.forEach((metaRoles: any) => {
				userInfos.value.role_info.forEach((roles: any) => {
...
export function setCacheTagsViewRoutes() {
	...
	const { userInfos } = storeToRefs(stores);
	let rolesRoutes = setFilterHasRolesMenu(dynamicRoutes, userInfos.value.role_info);
...
export function setFilterMenuAndCacheTagsViewRoutes() {
	...
	const { userInfos } = storeToRefs(stores);
	storesRoutesList.setRoutesList(setFilterHasRolesMenu(dynamicRoutes[0].children, userInfos.value.role_info));
